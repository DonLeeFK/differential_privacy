...
def LR_GD(X, Y, eps, delta, T, C = 1., eta = 0.1):  # Solve the Linear regression with (eps, delta)-differentially private SGD
    ...
    for i in range(T):
        tmp = np.dot(X,w)-Y
        gradient = 2*np.dot(X.T, tmp)          
        gradient = gradient + np.random.normal(0,sigmasq,gradient.shape)
        gradient = gradient/(max(1,np.linalg.norm(gradient, ord=2)))
        w = w - eta*gradient
        #print(w)
    return w
...
def LR_FM(X, Y, eps, delta):
    ...
    Phi_hat = Phi + noise_1
    # * Phi_hat can be modified approximately
    XY = np.dot(X.T, Y)
    XY_hat = XY + noise_2
...
if (__name__=='__main__'):
    ...
    for i in range (100):
        w = LR_GD(X, Y, eps, delta, T)
        loss1.append(L2_loss(X,Y,w))
        w = LR_FM(X, Y, eps, delta)
        loss2.append(L2_loss(X,Y,w))
